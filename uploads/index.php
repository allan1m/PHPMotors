...
switch ($action) {
    case 'upload':
        ...
        /**
         * When an image is uploaded it must be associated with a vehicle in the inventory table, the item's id is sent to the controller and 
         * stored. In addition, if the image is going to be the primary image, then that indicator is also sent to be stored.
         */
        ...
        // A model-based function is called to check if an image by the name of the uploaded image already exists in the database table.
        $imageCheck = checkExistingImage($imgName);
        
        
        if($imageCheck){
            // If the name check returns TRUE then an error message is set.
            $message = '<p class="notice">An image by that name already exists.</p>';
        } elseif (empty($invId) || empty($imgName)) {
            // If one is missing, the error message is set.
